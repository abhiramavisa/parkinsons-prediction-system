...
def extract_features(audio_path):
    """
    Extract vocal features from audio file.
    Returns raw parselmouth values - no calibration needed,
    values are already in UCI dataset range.
    """
    ...
    s_dda   = s_apq3 * 3.0
    s_db    = float(-20.0 * np.log10(1.0 - s_local)) if 0 < s_local < 1 else 0.0
    ...
    features = [
        ...
        fo_min,   # MDVP:Flo(Hz)
        j_local,  # MDVP:Jitter(%)
        j_abs,    # MDVP:Jitter(Abs)
        j_rap,    # MDVP:RAP
        j_ppq,    # MDVP:PPQ
        j_ddp,    # Jitter:DDP
        s_local,  # MDVP:Shimmer
        s_db,     # MDVP:Shimmer(dB)
        s_apq3,   # Shimmer:APQ3
        s_apq5,   # Shimmer:APQ5
        s_apq11,  # MDVP:APQ
        s_dda,    # Shimmer:DDA
    ...
    print(f"Features: Fo={fo_mean:.1f}Hz | Jitter={j_local:.5f} | Shimmer={s_local:.5f} | HNR={hnr:.2f}dB")
